...
public:
	...
	conv::pos3D<T> operator - (const Vertex<T>& other);
	operator bool() const;
};
...
template<typename T>
inline conv::pos3D<T> conv::Vertex<T>::operator-(const Vertex<T>& other)
{
	auto x = m_position[0] - other.getPos_X();
	auto y = m_position[1] - other.getPos_Y();
	auto z = m_position[2] - other.getPos_Z();

	return conv::pos3D<T>{x, y, z};
}

template<typename T>
conv::Vertex<T>::operator bool() const
{
	return (getPos_X()!=0 ) && (getPos_Y()!=0) && (getPos_Z()!= 0);
}
